...
Sparse CSR-матрица (1 280 329 × 225 427), density ≈ 0.00068%.
Матрица экстремально разреженная: медиана событий на пользователя = 1.
...
|---|---|---|
| recall@10 | **0.0167** | +11.3% |
| NDCG@10 | **0.0104** | +6.5% |
| precision@10 | **0.0028** | +16.7% |

Низкие абсолютные значения объясняются экстремальной разреженностью (density 0.00068%) и
14-дневным holdout: 97.4% пользователей имеют единственный addtocart в val-периоде.
ALS значимо улучшает результат для пользователей с историей (recall может достигать 0.27).
